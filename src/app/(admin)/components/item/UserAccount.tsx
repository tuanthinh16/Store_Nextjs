...
export const  BioCard=()=> {
    ...
    const fetchData =()=>{
        ...
        .then(response =>{
            ...
            console.log("data lay user: ",response.data['data'])
        })
        .catch(error=>{
    ...
    return (
        ...
        {user?.map((item:any,index:number)=>(
            ...
            <CardContent sx={{ alignItems: 'center', textAlign: 'center' }}>
                ...
                {item.role == 'admin'?(
                    <Typography level="title-lg" sx={{color:'red',fontWeight:'bold'}}>{item.name.firstname}{" "}{item.name.lastname}</Typography>
                ):(
                    item.role=='user'?(
                        <Typography level="title-lg" sx={{color:'gray',fontWeight:'bold'}}>{item.name.firstname}{" "}{item.name.lastname}</Typography>
                    ):(
                        item.role =='shop'?(
                            <Typography level="title-lg" sx={{color:'green',fontWeight:'bold'}}>{item.name.firstname}{" "}{item.name.lastname}</Typography>
                        ):(
                            <Typography level="title-lg" sx={{color:'blue',fontWeight:'bold'}}>{item.name.firstname}{" "}{item.name.lastname}</Typography>
                        )
                    )
                )}
                <Typography level="body-sm" sx={{ maxWidth: '24ch' }}>
                    This is {item.role} user
                </Typography>
                <Box
                sx={{
                    display: 'flex',
